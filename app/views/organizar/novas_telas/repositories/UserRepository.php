<?php 
    ...
    class UserRepository {
        ...
        public function create(UserModel $user) : int {
            try {
                $query = "INSERT INTO usuarios (nome, telefone, email, senha, username, dataNascimento, tipoUsuario) VALUES (:nome, :fone, :email, :senha, :username, :dataNascimento, :tipoUser)";
        ...
        public function update(UserModel $user) : bool {
            $query = "UPDATE usuarios SET nome = ?, telefone = ?, email = ?, senha = ?, username = ?, dataNascimento = ?, tipoUser = ? WHERE id = ?";
            $prepare = $this->conn->prepare($query);
            ...
            $prepare->bindValue(6, $user->getDataNascimento());
